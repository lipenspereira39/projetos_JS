...
const perguntas = [
    ...
    { id: 4, pergunta: 'Qual o ano da morte de Adolf Hitler?', resposta: '1945' },
    ...
    { id: 31, pergunta: 'Qual o ano da morte de Michael jackson?', resposta: '2009' },
    { id: 32, pergunta: 'Em que ano o Brasil foi tricampeão mundial de futebol?', resposta: '1970' },
    { id: 33, pergunta: 'Em que ano foi declarado morto Osama Bin Laden?', resposta: '2011' },
    { id: 34, pergunta: 'Em que ano foi lançado o filme De Volta para o Futuro?', resposta: '1985' },
    { id: 35, pergunta: 'Em que ano foi lançado o livro Alice no país da maravilhas', resposta: '1865' },
    { id: 36, pergunta: 'Qual o ano de fundação da Disney', resposta: '1923' },

  ];

  // Funções da aplicação

  function selecionarPerguntasAleatorias(perguntas, quantidadePerguntas){
    const perguntasAleatorias = perguntas.sort(() => Math.random() - 0.5);
    return perguntasAleatorias.slice(0, quantidadePerguntas);
  }
...
const perguntasSelecionadas = selecionarPerguntasAleatorias(perguntas, quantidadePerguntas);

let pontuacaoFinal = 0;

perguntasSelecionadas.forEach((pergunta, index) => {
    const respostaUsuario = exibirPergunta(pergunta, index);
    const validacaoResultado = validarRespostaUsuario(respostaUsuario, pergunta);
